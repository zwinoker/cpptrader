...
inline double stdv(std::vector<double> data, double mean) {
	double stdv = 0.0;
	int size = data.size();
	for (int i = 0; i < size; i++) {
...
inline std::vector<double> intervals(std::vector<double> data) {
...
inline boost::math::normal_distribution<> normal_ml_fit(std::vector<double> jumps) {
	int mcs = 10000;
	double paramnum = 0.0;
...
inline double mc_future_price_change(double mean, double std, int numsteps) {
